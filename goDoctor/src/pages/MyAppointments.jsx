...
const MyAppointments = () => {
  ...
  const appointmentRazorpay = async (appointmentId) => {
    try {
      ...
      if (data.success) {
        ...
        console.log(
          "success kaadu faliure myappointment.jsx lo appointment razorpay"
        );
      }
    } catch (error) {
      console.log(error);
  ...
  if (appointments.length === 0) {
    return (
      <p className="text-center font-bold text-orange-500 p-4">
        Lemme see 🔎! Looks like There is Nothing here....
      </p>
    );
  } else {
    return (
          ...
          {appointments.slice().map((item, index) => (
            ...
            >
              ...
              <div className="flex flex-col gap-2 justify-center">
                {item.payment ? (
                  <button
                    className="text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-primary hover:text-white transition-all duration-300 disabled:cursor-not-allowed disabled:bg-gray-200 disabled:hover:text-black"
                    disabled={item.cancelled || item.payment}
                  >
                    Paid
                  </button>
                ) : item.isCompleted ? (
                  <button className="font-semibold  text-center sm:min-w-48 py-8 border rounded text-green-800 hover:bg-green-800 hover:text-white transition-all duration-300">
                ...
                {item.cancelled ? (
                  <button className="text-sm  text-center sm:min-w-48 py-2 border rounded text-red-800 hover:bg-red-700 hover:text-white transition-all duration-300">
                    Apointment Cancelled
                  </button>
                ) : item.isCompleted ? (
                  <></>
                ) : (
